...
(function(globalObj){
    ...
    if (typeof module === 'object' && module.exports){
        module.exports = export_uniSoc_common
    }else {
    	globalObj.uniSoc=export_uniSoc_common(globalObj); 
    	//NOTE: We set the same property as uniSoc_web, that way we're only left with one
    	//       after both files have loaded.
    }

	function export_uniSoc_common(dep={}){

		function missingDependency(which){throw new Error("Missing dependency for uniSoc: "+which);}
		const cX = dep.cX || (dep.util && dep.util.cX ? dep.util.cX:null) || dep.BetterUtil  || missingDependency('BetterUtil');
	...
	/*2020-03-18: Instead of having this class rely in any way on the smart class, we've added 3 "middleware" oppertunities 
		...
		*/
		uniSoc.prototype.registerEndpoint=function registerEndpoint(subject,func,options={}){
			try{
				var self=this;
				cX.checkTypes(['string','function','object'],[subject,func,options]);
				...
				if(!argNames){
					try{
						argNames=cX.getArgNames.call(this,func,true); //true==show default values
					}catch(err){
				...
				if(Array.isArray(argNames)){
					...
					argNames=cX.filterSplit(argNames,arg=>arg.match(/(callback|payload|unisoc)/)==null,'retainIndex');
					...
					try{
						...
						switch(cX.varType(payload.data)){
							...
							case 'undefined':
								...
								break;
							default:
								this.log.makeError("BUGBUG: unexpected payload.data: ",cX.logVar(payload.data)).throw('EINVAL');
						}
						...
						p=cX.callInPromise(func,argsArr,options.callAs)
			...
			if(typeof endpoints.getEndpoint=='function'&&typeof endpoints.listVisibleEndpoints=='function'){
				...
				for(ep in endpoints){
					if(endpoints[ep].hasOwnProperty('listener')){
						clean[ep]=cX.extract(endpoints[ep],['listener','description','args','visible']);
					}
				}
		...
		async function preparePayload(payload){
			try{
				...
				if(typeof payload.subject!='string')
					this.log.makeError('A string subject is required, got:',cX.logVar(payload.subject)).throw('EINVAL');
		...
		*/
		function prepareRequest(responseCallback=undefined,timeout=0){
			...
			if(typeof responseCallback=='function'){
				...
				logStr+='promise'
				var {reject:requestFailed,promise:responsePromise,callback:responseCallback}=cX.exposedPromise();
		...
		*/
		uniSoc_Websocket.prototype._transmit=function(payload){
			...
			this.log.trace("Transmitting...",cX.logVar(str,50,'noLog'));
